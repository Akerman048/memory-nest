...
import { Memory, MemoryKind, memoryKindLabels } from "./types";
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-primary/35 p-0 backdrop-blur-sm sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="add-memory-title">
      ...
      <form onSubmit={submit} className="relative max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-t-[32px] bg-background p-6 shadow-2xl sm:rounded-[32px] sm:p-8">
        ...
        <div className="mt-7 grid grid-cols-4 gap-2">
          {kinds.map((item) => { const KindIcon = memoryKindIcons[item]; return <button type="button" key={item} onClick={() => { setKind(item); setMediaUrl(""); setMediaName(""); }} className={`rounded-2xl px-2 py-3 text-xs font-semibold transition sm:text-sm ${kind === item ? "bg-primary text-primary-foreground" : "bg-primary-soft text-muted"}`}><KindIcon className="mx-auto mb-1 text-xl" aria-hidden="true" />{memoryKindLabels[item]}</button>; })}
        </div>
        ...
        <div className="mt-7 flex justify-end gap-3"><button type="button" onClick={onClose} className="rounded-full px-5 py-3 font-semibold text-muted">Cancel</button><button disabled={Boolean(error)} className="flex items-center gap-2 rounded-full bg-primary px-7 py-3 font-semibold text-primary-foreground hover:bg-primary-hover disabled:opacity-50"><FiPlus aria-hidden="true" />Save memory</button></div>
      </form>
    </div>
  );
}
